...
sample_sizes <- c(30, 50, 100, 150, 200, 250, 300, 500)
df_values <- c(5)
...
```{r}
ggplot(data=Output, aes(x=n, y=ML, group=1)) +
  geom_line()+
  geom_point()
```
...
```{r}
ggplot(Output.melt, 
       aes(x=n, y=Estimates, fill=Estimator)) +
...
```{r}
bootstrap.median <- function(x, B){
  sample_size <- length(x)
  t <- median(x)/log(2)
  
  if(sample_size^sample_size<B) {
        m.Bootstrap <- permutations(n=sample_size, 
                                    r=sample_size, 
                                    v=x, 
...
```{r}
ggplot(summary.information, 
       aes(x=n, y=mean, colour=Estimator, group=Estimator)) + 
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_line() +
  geom_point(size=3, shape=21, fill="white") +
  xlab(expression(italic(n))) +
  ylab("Mean and Error Bars")
```
